...
::: tip AI 模型配置的分工
密钥配置页中的「AI 聊天 · 内置默认模型」「DeepSeek 模型」「Tavily 搜索 API」三组是**基础设施层**：为模型注册表的内置行提供兜底地址与密钥。接入/停用/切换更多 AI 模型（GLM、Qwen、Kimi 等）请在 CI 的 **AI 应用 → 模型管理**面板完成，无需重启服务。详见 [配置项 → AI 配置](/deployment/configuration#ai-配置)。
:::
