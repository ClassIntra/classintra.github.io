...
`pre-records.json` 是**唯一名单源**——启动时由它全量重建 `pre_records`，setup 页保存时也会**整体覆盖**它。因此名单文件一旦被写坏，所有未注册学生都会立即收到「该姓名不在预注册名单中」。

`setup /save` 已内置两道防护：

1. **骤减拦截**：新名单人数从 ≥10 骤减至不足原有的 50% 时拒绝写入，需在请求中显式传 `force: true` 才继续
2. **自动备份**：覆盖前按天备份为 `pre-records.bak-YYYY-MM-DD.json`（同日只备一份）

::: danger 名单文件是隐私数据
`pre-records*.json`（含备份）已被 `.gitignore` 与 `.stignore` 双重排除——不提交仓库、不跨机同步。新增任何派生文件都必须沿用 `pre-records<后缀>.json` 命名，否则会被同步/提交，造成学生真实姓名外泄。
:::

### 名单损坏后的恢复

若名单被覆盖或误改，按以下顺序恢复（推荐从最接近事故时间点的来源取数据）：

1. **DB 备份的 `pre_records` 表**（最可靠）：`backup/*/database/*.db` 或 `backup/db-snapshot/*.db` 里直接读 `SELECT user_id, real_name, gender FROM pre_records`
2. **交叉校验**：把备份名单与现网 `users` 表（已注册用户）比对——ID↔姓名不一致必须人工确认后再写入；「备份有、现网无」的条目即为待恢复的未注册学生
3. **写入**：更新 `pre-records.json`，并同步 `pre_records` 表（`DELETE` 后按 json 全量插入，与启动导入行为一致）
4. **复核**：重启服务后确认 `pre_records` 条数正确，并抽查关键人员

::: tip 快速判断影响面
任何未注册学生都会遇到同样提示。排查时先数一下 `SELECT COUNT(*) FROM pre_records`——正常值应与班级名单人数一致，只剩寥寥几条即为名单已被覆盖。
:::
